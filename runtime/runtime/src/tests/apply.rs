...
fn test_apply_no_op() {
    let (runtime, tries, root, apply_state, _, epoch_info_provider) =
        setup_runtime(vec![alice_account()], to_yocto(Balance::from_near(1_000_000)), Balance::ZERO, Gas::from_tera(1000));
    runtime
        .apply(
...
fn test_apply_surplus_gas_for_transfer() {
    ...
    let expected_burnt_amount = if fees.refund_gas_price_changes {
        GAS_PRICE.checked_mul(exec_gas.as_gas().into()).unwrap()
    } else {
        gas_price.checked_mul(exec_gas.as_gas().into()).unwrap()
    };
...
fn test_compute_usage_limit() {
    let (runtime, tries, mut root, mut apply_state, signers, epoch_info_provider) =
        setup_runtime(vec![alice_account()], to_yocto(Balance::from_near(1_000_000)), to_yocto(Balance::from_near(500_000)), Gas::ONE);
